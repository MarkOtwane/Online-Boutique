...
import { OrderUpdatesService, OrderStatusUpdate, OrderTrackingUpdate } from '../services/order-updates.service';
...
export class OrderHistoryComponent implements OnInit, OnDestroy {
  ...
  private setupRealTimeUpdates(): void {
    ...
    const statusSub = this.orderUpdatesService.orderUpdates$.subscribe(updates => {
      this.recentUpdates = updates;
      this.updateOrdersWithUpdates(updates);
    });

    // Subscribe to tracking updates
    const trackingSub = this.orderUpdatesService.trackingUpdates$.subscribe(updates => {
  ...
  autoTrackOrder(orderId: number): void {
    ...
    this.http.get<any>(`http://localhost:3000/orders/${orderId}/tracking`, { headers }).subscribe({
      next: (data: any) => {
  ...
  shouldShowTrackingButton(order: any): boolean {
    ...
    return !this.isAdmin && (status === 'shipped' || status === 'completed' || status === 'delivered');
  }
